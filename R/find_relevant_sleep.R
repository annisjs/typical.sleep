...
find_relevant_sleep <- function(all_sleep_dat)
{
    ...
    # Setup ranges
    # Relevant sleep logs fall within +/- 8 hours of the median msp
    first_last_asleep[, median_msp_date := ymd(as_date(start_datetime)) + hms(median_msp)]
    first_last_asleep[, range_begin := median_msp_date - hours(8)]
    first_last_asleep[, range_end := median_msp_date + hours(8)]
    ...
    first_last_asleep[, range_begin2 := range_begin + days(1)]
    first_last_asleep[, range_end2 := range_end + days(1)]
    first_last_asleep[, range_begin3 := range_begin - days(1)]
    first_last_asleep[, range_end3 := range_end - days(1)]
    ...
    # Find all overlapping sleep logs within +/- 8 hours of the MSP
    ...
    # Keep relevant sleep logs overlapping with MSP +/-8 hours
    all_sleep_dat[, msp := FALSE]
    all_sleep_dat[dt_overlaps$xid, msp := TRUE]
    all_sleep_dat[dt_overlaps$xid, date_new := as_date(dt_overlaps$date)]
